...
ASTvehiclesR = ASTvehicles apply {_x = (_x select 0);};

ASTvehiclesR append ["I_Truck_02_ammo_F","O_Truck_02_Ammo_F","O_Truck_03_ammo_F"];

ASTdropvehicles = ASTvehicles apply {(_x select 1) = (_x select 1) + 30;};
...
ASTAirListR = ASTairlist apply {_x = (_x select 0);};
...
ASTdropVehiclesSpawnList = ASTvehiclesSpawnList apply {(_x select 1) = [format ["Points: %1",(_x select 1) + 30]]};
