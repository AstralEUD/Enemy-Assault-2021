...
CRS_HolsterEH=(findDisplay 46)displayAddEventHandler["KeyDown","if(_this select 1==12)then{call CRS_Holster}"];};
...
titleText["<t size='3' color='#ff0000' shadow='3'>총기 탄걸림</t>","PLAIN DOWN",1,true,true];titleFadeOut 5;
};}];
};
...
//(findDisplay 46)displayAddEventHandler["KeyDown","if(inputAction""Headlights""==0.5)then{if((currentWeapon player==primaryWeapon player)&&{((primaryWeaponItems player)select 1)!=""""})};"];
